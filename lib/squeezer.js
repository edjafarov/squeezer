...
function squeeze(obj, path, options, callback){
  ...
  var pathArray = path.split(".");
  if(_.isArray(obj)) {
    obj = squeezeArray(obj, pathArray[0]);
    _.each(obj, function(item, val){
      obj[val] = squeeze(item, pathArray.slice(1).join("."), options);
    })
    
  }else{
    var next = pathArray.shift();
    if(options.zip && _.isArray(obj[next])){
      if(obj[next]) {
        var tmpobj = squeeze(obj[next], pathArray.join("."), options);
        delete obj[next];
        _.extend(obj,tmpobj);
      }
    }else{
      if(options.zipProps && options.zipProps.indexOf(next)!==-1) {
        if(callback) return callback(null,obj[next]);
        return obj[next];
      }
 
      if(obj[next]) obj[next] = squeeze(obj[next], pathArray.join("."), options);
